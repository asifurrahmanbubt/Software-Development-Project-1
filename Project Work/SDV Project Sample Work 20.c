...
void view_tasks(struct Month* m) {
    ...
    if (m->tasks[index] != NULL) {
        ...
        printf("4. View tasks again (by date)\n");
        printf("0. Exit\n");
        printf("Enter option: ");
        int option;
        scanf("%d", &option);
        ...
        switch (option) {
            ...
            case 3:
                ...
                break;
            case 4:
                view_tasks(m); // Recursive call to view tasks by date again
                break;
            default:
...
bool search_tasks(struct Month* m, const char* search_term) {
    bool found = false;
    for (int i = 0; i < m->num_days; i++) {
        if (m->tasks[i] != NULL && strstr(m->tasks[i], search_term) != NULL) {
            if (!found) {
                printf("Tasks containing '%s':\n", search_term);
                found = true;
            }
            printf("Day %d: %s\n", i + 1, m->tasks[i]);
        }
    ...
    while (1) {
        ...
        if (choice == 'y' || choice == 'Y') {
            while (1) {
                ...
                if (!search_tasks(&m, search_term)) {
                    printf("No tasks found containing '%s'\n", search_term);
                    printf("Do you want to add '%s' as a task? (y/n): ", search_term);
                    scanf(" %c", &choice);

                    if (choice == 'y' || choice == 'Y') {
                        printf("Enter the day to add the task: ");
                        scanf("%d", &day);
                        if (day < 1 || day > m.num_days) {
                            printf("Invalid day!\n");
                        } else {
                            add_task(&m, day, search_term);
                            printf("Task added successfully!\n");
                        }
                    }
                    ...
                    printf("Do you want to change the date of any of these tasks? (y/n): ");
                    scanf(" %c", &choice);
